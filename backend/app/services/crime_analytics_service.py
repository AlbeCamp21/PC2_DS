...
from app.models.crime_stats import DistrictCrimeStats, DistrictCrimeStatsBuilder
...
class CrimeAnalyticsService:
    ...
    def process_crime_metrics(self):
        # Transforma datos en indices seguridad
        # Extrae los datos crudos
        ...
        # Aplica ponderacion de peligrosidad
        ...
        # Agrega toda historia unificada
        ...
        # Normaliza las metricas obtenidas
        ...
        # Persiste todas metricas condensadas
    ...
    def update_stats_table(self, stats_df):
        # Limpia actualiza tabla de estadisticas
        self.db.query(DistrictCrimeStats).delete()
        
        for _, row in stats_df.iterrows():
            builder = DistrictCrimeStatsBuilder()
            stat_entry = (builder
                .set_ubigeo(row['district_ubigeo'])
                .set_name(row['district_name'])
                .set_total_incidents(int(row['total_all']))
                .set_violent_incidents(int(row['total_violent']))
                .set_weighted_rate(float(row['final_rate']))
                .build())
